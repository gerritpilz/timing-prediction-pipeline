...
if __name__ == "__main__":
    ...
    for design_clk in os.listdir(args.build_dir):
        ...
        logging.info(f"direction       unique:  {dataset.x[:, 1].unique().tolist()}")
        logging.info(f"log1p(fanout)   min/max: {dataset.x[:, 2].min():.0f} / {dataset.x[:, 2].max():.0f}")
        logging.info(f"input_depth     max:     {dataset.x[:, 3].max():.0f}")
        logging.info(f"output_depth    max:     {dataset.x[:, 4].max():.0f}")
        logging.info(f"is_clk          unique:  {dataset.x[:, 5].unique().tolist()}")
        logging.info(f"pin_cap         min/max: {dataset.x[:, 6].min():.4f} / {dataset.x[:, 6].max():.4f}")

        # targets
        logging.info(f"slack_min    min/max: {dataset.y[:, 0].min():.4f} / {dataset.y[:, 0].max():.4f}")
        logging.info(f"slew_r       min/max: {dataset.y[:, 1].min():.4f} / {dataset.y[:, 1].max():.4f}")
        logging.info(f"slew_f       min/max: {dataset.y[:, 2].min():.4f} / {dataset.y[:, 2].max():.4f}")
        ...
        # vorteil modell: evaluation in ms vs run in minuten -> für routing 1000tris*minuten sehr viel!
